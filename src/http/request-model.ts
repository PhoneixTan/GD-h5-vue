...
// 默认的请求方式
export const DEFAULT_REQUEST_HEADER = new RequestHeaderModel()

/**
 * 设置默认的请求头信息
 */
export const setDefaultHeaderAuthToken = () => {
	DEFAULT_REQUEST_HEADER.put('authToken', () => {
		const systemStore = useSystemStore()
		return systemStore.token
	})
}
 ...
 * 设置默认的表单防重内容
 */
export const setDefaultHeaderSubmitKey = () => {
	DEFAULT_REQUEST_HEADER.put('submitKey', uuid)
}
