'use strict'
import { isSafari, isTrident } from './browser-util'
 ...
 */
export const formatByTimestamp = (value = 0, format = 'hh:mm:ss.SSS') => {
 ...
 * @returns {TimeModel}
 */
export const formatByTime = (value = 0, format = 'hh:mm:ss.SSS') => {
 ...
 * @returns {DateModel}
 */
export const formatByDate = (value = new Date(), format = 'yyyy-MM-dd HH:mm.ss.SSS') => {
 ...
 * @returns {DateModel}
 */
export const formatByString = (date: string, format = 'yyyy-MM-dd HH:mm.ss.SSS') => {
 ...
 */
export const formatSubDate = (dateTime: string | undefined, needSlash?: boolean): string => {
 ...
 * @param needSlash 是否需要斜杠
 */
export const formatSubDateTime = (dateTime: string | undefined, needSlash?: boolean): string => {
 ...
 * @param length
 */
export const formatDateTimeWithSub = (dateTime: string | undefined, length: number, needSlash?: boolean): string => {
	if (!dateTime) return ''
...
export class DateModel {
	...
	_renderValue(value: Date | string = new Date()) {
		value = typeof value === 'string' && (isSafari() || isTrident()) ? value.replace(/-/g, '/') : value
 ...
 * 计时工具
 */
export class TimeCountModel implements TimeCountType {
	o?: NodeJS.Timer | null
	time: number
	isStart?: boolean
	isFinish?: boolean
